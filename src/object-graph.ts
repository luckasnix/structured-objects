export class ObjectGraph<NodeValue extends Record<string, unknown>> {
	...
	public match(
		...
		const matchedNodes: Array<NodeValue> = [];
		for (const [_, nodeValue] of this.nodes) {
			const shapeEntries = Object.entries(shape) as Array<
				[keyof NodeValue, unknown]
			>;
			const hasMatched = shapeEntries.every((shapeEntry) => {
				...
				const shapePropValueArray: Array<unknown> = Array.isArray(
					shapePropValue,
				)
					? shapePropValue
					: [shapePropValue];
